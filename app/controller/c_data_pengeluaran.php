<?php
	if(isset($_POST['via_ajax'])){
		include "../model/m_data_pengeluaran.php";
		include "../config/config.php";
		include "../model/connect.php";
		
	}else{
		include "app/model/m_data_pengeluaran.php";
	}
	
	$data_pengeluaran	= new C_data_pengeluaran();
	if(isset($_POST['func_d'])){
		if(isset($_GET['p'])){
			$data = $data_pengeluaran->$_POST['func_d']($_GET['p']);
		}else{
			$data = $data_pengeluaran->$_POST['func_d']();
		}
	...
	class C_data_pengeluaran{
		...
		var $keterangan;
		var $message;
		var $m_data_pengeluaran;
		
		function __construct(){
			$this->m_data_pengeluaran	= new M_data_pengeluaran();
		}
		...
		function input_pengeluaran(){
			...
			if(!in_array("",$required_data, TRUE)){
				...
				$result			= $this->m_data_pengeluaran->insert_data($this);
		...
		function get_all_data(){
			$data	= $this->m_data_pengeluaran->select_all();
			if($data->status){
				return $data->data;
		...
		function get_pengeluaran_by_category($condition){
			...
			$data				= $this->m_data_pengeluaran->select($this);
		}
